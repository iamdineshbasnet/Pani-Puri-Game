...
const progressPercentageElement = document.querySelector(".progress-wrapper .progress-percentage")
...
function crushPuri() {
  ...
  if (src.includes(missionOptions.type[0]) && puriCrushed === missionOptions.goal) {
...
function createMission() {
  ...
  const allPossibleMissions = getAllPossibleMissions(randomRow, randomColumn, puriSrc);
  ...
  if (allPossibleMissions.length > 0) {
    ...
    const missionTypes = allPossibleMissions.map((cell) =>
      cell.src.split('/').pop().split('.')[0]
    );
